...
fig_scatter.update_layout(width=1000, height=1000)
...
inertias_df = pd.DataFrame({
    'Number of Clusters': list(range(1,7)),
    'Inertia': inertias[0:6]
})
line_chart = px.line(inertias_df,
...
with col1:
    ...
    fig = px.scatter(df, 
                     ...
                     color="Cluster",
                     category_orders={'Cluster': sorted(df['Cluster'])}
                     )
    st.plotly_chart(fig)
